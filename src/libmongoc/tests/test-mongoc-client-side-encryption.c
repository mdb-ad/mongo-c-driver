...
explicit_encryption_setup_full(const char *encrypted_fields_path, const char *key_path)
{
   ...
   /* Create a MongoClient named ``autoEncryptedClient``. */
   {
      ...
      ASSERT_OR_PRINT(mongoc_client_enable_auto_encryption(eef->autoEncryptedClient, aeOpts, &error), error);
...
test_explicit_encryption_text_prefix_suffix_ci_di(void *unused)
{
   ...
   /* Case 8: insert 'BingQiLin' via autoEncryptedClient */
   {
      bson_error_t ae_err;
      mongoc_collection_t *ae_coll =
         mongoc_client_get_collection(eef->autoEncryptedClient, "db", "explicit_encryption");
      ASSERT_OR_PRINT(
         mongoc_collection_insert_one(
            ae_coll, tmp_bson("{ 'encryptedText': 'BingQiLin' }"), NULL /* opts */, NULL /* reply */, &ae_err),
         ae_err);
   ...
   /* Case 9: insert 'cafébarbäz' via autoEncryptedClient */
   {
      mongoc_collection_t *ae_coll =
         mongoc_client_get_collection(eef->autoEncryptedClient, "db", "explicit_encryption");
...
static void
test_explicit_encryption_text_substring_ci_di(void *unused)
{
   bson_error_t error;
   bool ok;
   bson_value_t plaintext = {0};
   ...
   /* Case 10: insert 'FooBarBaz' via autoEncryptedClient and find case-insensitively */
   {
      mongoc_collection_t *ae_coll =
         mongoc_client_get_collection(eef->autoEncryptedClient, "db", "explicit_encryption");
      ASSERT_OR_PRINT(
         mongoc_collection_insert_one(
            ae_coll, tmp_bson("{ 'encryptedText': 'FooBarBaz' }"), NULL /* opts */, NULL /* reply */, &error),
         error);
      mongoc_collection_destroy(ae_coll);
   }
   ...
   /* Case 11: insert 'foocafébaz' via autoEncryptedClient and find diacritic-insensitively */
   {
      mongoc_collection_t *ae_coll =
         mongoc_client_get_collection(eef->autoEncryptedClient, "db", "explicit_encryption");
      ASSERT_OR_PRINT(mongoc_collection_insert_one(ae_coll,
                                                   tmp_bson("{ 'encryptedText': 'foocaf\xc3\xa9"
                                                            "baz' }"),
                                                   NULL /* opts */,
                                                   NULL /* reply */,
                                                   &error),
...
test_client_side_encryption_install(TestSuite *suite)
{
   ...
   // Add test cases for prose test: 22. Range Explicit Encryption.
   {
      ...
      TestSuite_AddFull(suite,
                        ...
                        test_framework_skip_if_no_client_side_encryption);
      TestSuite_AddFull(suite,
                        "/client_side_encryption/explicit_encryption/text/substring_ci_di",
                        test_explicit_encryption_text_substring_ci_di,
                        NULL,
                        NULL,
                        test_framework_skip_if_max_wire_version_less_than_27 /* require server > 8.2 for QE support */,
                        test_framework_skip_if_single, /* QE not supported on standalone */
